            ...
            <form action="{{ route('admin.branches.update', $branch) }}" method="POST" class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $branch->name) }}" required class="input-field">
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div>
                        <label for="code" class="label-field">Branch Code <span class="text-red-500">*</span></label>
                        <input type="text" name="code" id="code" value="{{ old('code', $branch->code) }}" required class="input-field">
                        <x-input-error :messages="$errors->get('code')" class="mt-2" />
                    </div>

                    <div class="md:col-span-2">
                        <label for="address" class="label-field">Address</label>
                        <textarea name="address" id="address" rows="3" class="input-field">{{ old('address', $branch->address) }}</textarea>
                        <x-input-error :messages="$errors->get('address')" class="mt-2" />
                    </div>

                    <div>
                        <label for="email" class="label-field">Email Address</label>
                        <input type="email" name="email" id="email" value="{{ old('email', $branch->email) }}" class="input-field">
                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                    </div>

                    <div>
                        <label for="phone" class="label-field">Phone Number</label>
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $branch->phone) }}" class="input-field">
